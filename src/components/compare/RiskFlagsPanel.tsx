...
/** Collect the flags of every scenario, merged by flag id + title so each risk appears once with the routes it applies to. */
function groupFlags(results: ScenarioResult[]): Record<Severity, GroupedFlag[]> {
  ...
  for (const r of results) {
    for (const f of r.flags) {
      ...
      if (existing) {
        if (!existing.scenarioNames.includes(r.definition.name)) existing.scenarioNames.push(r.definition.name)
      } else {
        map.set(key, { flag: f, scenarioNames: [r.definition.name] })
      }
    }
  }
...
export function RiskFlagsPanel({ results }: { results: ScenarioResult[] }) {
  ...
  if (total === 0) return <p className="text-sm text-slate-500">No risk flags were raised for these routes.</p>
                ...
                <li key={`${flag.id}-${flag.title}`} className={`rounded-lg border p-3 ${style.box}`}>
                  ...
                  {flag.caseStudyId && (
                    <button
                      ...
                      title="Open the Risks page for this case study"
                    >
